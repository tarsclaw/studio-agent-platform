...
const SERVICE_URL = () =>
  process.env.BOT_SERVICE_URL || "https://smba.trafficmanager.net/uk/";
...
export async function sendProactiveMessage(
  ...
  try {
    ...
    const msgRes = await fetch(
      `${serviceUrl}v3/conversations/${conv.id}/activities`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
 ...
 */
export function buildOnboardingMessage(employeeName: string, brand?: string): string {
