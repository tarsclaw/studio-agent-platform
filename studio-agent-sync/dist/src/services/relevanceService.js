...
exports.upsertMappings = upsertMappings;
exports.upsertSingleMapping = upsertSingleMapping;
...
async function getAllMappings() {
    ...
    while (true) {
        ...
        if (!cursor || data.results.length < pageSize)
            break;
        page++;
        if (page > 50)
            break; // safety valve
 ...
 */
async function upsertMappings(records) {
    let inserted = 0;
    let updated = 0;
    let failed = 0;
    const batchSize = 50;
    for (let i = 0; i < records.length; i += batchSize) {
        ...
        try {
            const res = await fetch(`${baseUrl()}/latest/knowledge/sets/${DATASET()}/documents/bulk_upsert`, {
                method: "POST",
                headers: headers(),
                body: JSON.stringify({
                    documents,
 ...
 */
async function upsertSingleMapping(record) {
 ...
 */
async function deleteMappings(aadObjectIds) {
    if (aadObjectIds.length === 0)
        return;
    const res = await fetch(`${baseUrl()}/latest/knowledge/sets/${DATASET()}/documents/bulk_delete`, {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({
            filters: [
